...
exports.mytickets = function(req, res){
  ...
  connection.connect(function(err) {
    ...
    if (err) throw err
      ...
      connection.query('SELECT Site, CallID, Category, CallStatus, Symptoms, TempDate FROM 395project.calllog WHERE CustID="' + req.user.id + '" and CallStatus="Closed";', function(err, result) {
        if (err) throw err
    ...
    console.log('closed');
    if (err) throw err
      connection.query('SELECT Site, CallID, Category, CallStatus, Symptoms, TempDate FROM 395project.calllog WHERE CallStatus="Open";', function(err, result) {
        if (err) throw err
    ...
    if (err) throw err
      connection.query('SELECT Site, CallID, Category, CallStatus, Symptoms, TempDate FROM 395project.calllog WHERE CustID="' + req.user.id + '";', function(err, result) {
        if (err) throw err
